...
#Optimize fixed effect structure using AIC
nd.global.model <- lmer(scale(length) ~ sex*temp_2_z + temp_100_z + sex:temp_100_z + sst_sd_z + sex:sst_sd_z + chla_z + sex:chla_z + sla_z + sex:sla_z + moci_spring_z + sex:moci_spring_z + cuti_z + sex:cuti_z + (1|station), data = ndc, na.action = na.fail, REML = FALSE)
...
#top model with full dataset with random slope and intercept for SST
nd.model <- lmer(scale(length) ~ chla_z + cuti_z + sex + sla_z + sst_sd_z + temp_100_z + temp_2_z + (1 | station) + cuti_z:sex, data = nd)
