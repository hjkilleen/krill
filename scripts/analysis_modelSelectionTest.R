...
ep.int <- lmer(scale(length) ~ sex*temp_2_z + temp_100_z + sex:temp_100_z + sst_sd_z + sex:sst_sd_z + chla_z + sex:chla_z + moci_spring_z + sex:moci_spring_z + cuti_z + sex:cuti_z + (1|station), data = epc, na.action = na.fail, REML = FALSE) #model with random intercept 

ep.intSlope <- lmer(scale(length) ~ sex*temp_2_z + temp_100_z + sex:temp_100_z + sst_sd_z + sex:sst_sd_z + chla_z + sex:chla_z + moci_spring_z + sex:moci_spring_z + cuti_z + sex:cuti_z + (1+temp_2_z|station), data = epc, na.action = na.fail, REML = FALSE)#model with random intercept and slope
...
ep.global.model <- lmer(scale(length) ~ sex*temp_2_z + temp_100_z + sex:temp_100_z + sst_sd_z + sex:sst_sd_z + chla_z + sex:chla_z + moci_spring_z + sex:moci_spring_z + cuti_z + sex:cuti_z + (1+temp_2_z|station), data = epc, na.action = na.fail, REML = FALSE)
...
#North vs South models for EP reveal little regional difference in response to environmental drivers
#=======
#N and S models
epN <- filter(ep[complete.cases(ep),], latitude >= 34.5)
epS <- filter(ep[complete.cases(ep),], latitude < 34.5)
epN.global.model <- lmer(scale(length) ~ sex*temp_2_z + temp_100_z + sex:temp_100_z + sst_sd_z + sex:sst_sd_z + chla_z + sex:chla_z + moci_spring_z + sex:moci_spring_z + cuti_z + sex:cuti_z + (1+temp_2_z|station), data = epN, na.action = na.fail, REML = FALSE)
epN.model.set <- dredge(epN.global.model)

epN.top.model <- get.models(epN.model.set, subset = 1)

epN.top.model
epN.model <-  lmer(scale(length) ~ chla_z + moci_spring_z + sex + sst_sd_z + temp_100_z + temp_2_z + (1 + temp_2_z | station) + sex:temp_100_z, data = epN)
sjPlot::plot_model(epN.model)

#boundary fit is singular with random slope
epS.global.model <- lmer(scale(length) ~ sex*temp_2_z + temp_100_z + sex:temp_100_z + sst_sd_z + sex:sst_sd_z + chla_z + sex:chla_z + moci_spring_z + sex:moci_spring_z + cuti_z + sex:cuti_z + (1|station), data = epS, na.action = na.fail, REML = FALSE)
epS.global.model2 <- lm(scale(length) ~ sex*temp_2_z + temp_100_z + sex:temp_100_z + sst_sd_z + sex:sst_sd_z + chla_z + sex:chla_z + moci_spring_z + sex:moci_spring_z + cuti_z + sex:cuti_z, data = epS, na.action = na.fail, REML = FALSE)

AIC(epS.global.model, epS.global.model2)

epS.model.set <- dredge(epS.global.model2)

epS.top.model <- get.models(epS.model.set, subset = 1)

epS.top.model
epS.model <- lm(scale(length) ~ chla_z + moci_spring_z + sex + sst_sd_z + temp_100_z + temp_2_z + sex:sst_sd_z + 1, data = epS)
sjPlot::plot_model(epS.model)
#=======
...
#Optimize random effects structure using maximum likelihood
ts.int <- lmer(scale(length) ~ sex*temp_2_z + sst_sd_z + sex:sst_sd_z + chla_z + sex:chla_z + moci_spring_z + sex:moci_spring_z + cuti_z + sex:cuti_z + (1|station), data = tsc, na.action = na.fail, REML = FALSE) #model with random intercept 

ts.intSlope <- lmer(scale(length) ~ sex*temp_2_z + sst_sd_z + sex:sst_sd_z + chla_z + sex:chla_z + moci_spring_z + sex:moci_spring_z + cuti_z + sex:cuti_z + (1+temp_2_z|station), data = tsc, na.action = na.fail, REML = FALSE)#model with random intercept and slope

anova(ts.int, ts.intSlope)#compare models with different random effect structure
#Slope intercept is the optimal random effects structure

#Optimize fixed effect structure using AIC
ts.global.model <- lmer(scale(length) ~ sex*temp_2_z + sst_sd_z + sex:sst_sd_z + chla_z + sex:chla_z + moci_spring_z + sex:moci_spring_z + cuti_z + sex:cuti_z + (1+temp_2_z|station), data = tsc, na.action = na.fail, REML = FALSE)
...
#Optimize fixed effect structure using AIC
nd.global.model <- lmer(scale(length) ~ sex*temp_2_z + temp_100_z + sex:temp_100_z + sst_sd_z + sex:sst_sd_z + chla_z + sex:chla_z + moci_spring_z + sex:moci_spring_z + cuti_z + sex:cuti_z + (1|station), data = ndc, na.action = na.fail, REML = FALSE)
