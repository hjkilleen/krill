...
a <- ggplot(allLengthsEnv, aes(x = as.factor(year), y = length.unscaled, group = year, fill = as.factor(year))) +
  ...
  geom_text(data = summarize(group_by_at(aLE_tally, vars(year, species)), parse = TRUE, mean = round(mean(length.unscaled), 1), max = max(length.unscaled)), aes(x = as.factor(year), y = max + 13, label = paste("mean=", mean, sep = " ")), color = "black", size = 4) +
...
a + 
ggsave("figures/manuscript/figure2_krillViolinCV.jpeg", width = 8, height = 9, dpi = 400)
#====
