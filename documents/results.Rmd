...
ggplot(epg, aes(x = temp_2, y = length)) + 
  geom_point(aes(color=sites))
```
...
```{r}
tsg.nosc <- lmer(scale(length) ~ chla_z + moci_spring_z + sex + sla_z + sst_sd_z + temp_100_z + temp_2_z + I(temp_2_z^2) + (1 + temp_2_z | station) + chla_z:sex + sex:sla_z + sex:sst_sd_z + sex:temp_100_z + sex:temp_2_z, data = filter(ts, sites != "SouthernCA", sites != "MidChannelIslands"))

sjPlot::plot_model(tsg.nosc)
```
...
```{r, results=FALSE, warning=FALSE}
nd.model <- lmer(scale(length) ~ chla_z + cuti_z + moci_spring_z + sex + sla_z + sst_sd_z + temp_100_z + temp_2_z + (1 | station) + cuti_z:sex + moci_spring_z:sex, data = nd)
